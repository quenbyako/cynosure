...
orchestration, located at `internal/apps/<context>/*`, where `<context>` is a application name,),
- **Presentation** (controllers/handlers, located at
`internal/controllers/<context>/*`, where `<context>` is a contract, how external services interact with the system. For example, `<context>` may be `http`, `grpc`, `telegram_bot`, etc. Context name **SHOULD** prefer naming on contract name over protocol, e.g. lay on grpc service name `state.v1.StaeteService` rather than `grpc`),
- **Infrastructure** (adapters implementing ports, located at
`internal/adapters/<context>/*`, where `<context>` is a service, subsystem, API, or anything similar. For example, `<context>` may be a dependency service name, or protocol).
...
All public methods, functions, types, fields, constants, and package-level
elements **MUST** have clear, concise GoDoc comments explaining their purpose. ALL PUBLIC API SURFACES **MUST** be documented, meaning that there **MUST NOT** be any undocumented exported identifiers. Comments **MUST** describe the behavior, parameters, return values, and any side effects or error conditions. Documentation **MUST** be kept up-to-date with code changes; outdated comments are considered worse than no comments.

If some method implements a specific interface, documentation for each method **MUST** be cloned from original interface definition to ensure clarity of intent. If this method has some additional behavior, it **MUST** be documented after the cloned comment with explicit note that documentation is somehow extended, but not too verbose.

Rationale: Facilitates onboarding, eases maintenance, and improves code comprehension.
...
To structurize code more clearly, data transfer objects and funtions (DTOs) **MUST** be stored in a subpackage named `datatransfer`, e.g. `internal/adapters/someapi/datatransfer/`. This package **MUST** contain only data transfer objects and functions that are converting only from adapter-specific types (generated by grpc code gen for example) into domain types, and vice versa, with consistent naming, e.g. `UserProfileToGRPC` and `UserProfileFromGRPC`. No other code is allowed in this package.

Datatransfer package **MUST NOT** be imported from any other adapter except the one it is defined in.
