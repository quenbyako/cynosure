...
func Cmd(ctx context.Context, appCtx goose.AppCtx[Flags]) int {
	_ = cynosure.NewApp(ctx,
		cynosure.WithObservability(appCtx.Log, appCtx.Metric, appCtx.Trace),
		cynosure.WithGRPCServer(appCtx.Config.Port),
		cynosure.WithHTTPServer(appCtx.Config.HttpPort.Register),
		cynosure.WithZepKey(appCtx.Config.ZepKey),
		cynosure.WithGeminiKey(appCtx.Config.GeminiKey),
