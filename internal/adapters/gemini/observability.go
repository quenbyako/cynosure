...
type observable struct {
	// stack is used, when adapter needs to construct port wrappers.
...
func (o *observable) streamStarted(ctx context.Context, model string, toolCount int) {
	o.event(ctx, log.SeverityInfo, "cynosure.adapters.gemini.stream_started").
		Context(
...
func (o *observable) tokenCountMismatch(ctx context.Context, model string, expected, got uint32) {
	o.event(ctx, log.SeverityWarn, "cynosure.adapters.gemini.token_count_mismatch").
		Context(
			attribute.String("model", model),
