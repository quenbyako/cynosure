...
func New(opts ...Option) *Guardian {
	guardian := &Guardian{
		...
		ports:             []uint16{80, 443},
		allowedv4Prefixes: nil,
		allowedv6Prefixes: nil,
		deniedv4Prefixes:  nil,
		deniedv6Prefixes:  nil,
	}
	...
	guardian.deniedv4Prefixes = append(guardian.deniedv4Prefixes, IPv4DeniedPrefixes...)
	guardian.deniedv6Prefixes = append(guardian.deniedv6Prefixes, IPv6DeniedPrefixes...)
...
func (g *Guardian) Safe(network, address string, _ syscall.RawConn) error {
	if err := g.validateNetwork(network); err != nil {
		return err
	}

	ipport, err := netip.ParseAddrPort(address)
	if err != nil {
		return fmt.Errorf("%w: could not parse %s: %w", ErrInvalidHostPort, address, err)
	}

	if err := g.validatePort(ipport.Port()); err != nil {
		return err
	}

	return g.validateIP(ipport.Addr())
}

func (g *Guardian) validateNetwork(network string) error {
	if g.networks != nil {
		if !slices.Contains(g.networks, network) {
			return fmt.Errorf(
				"%w: %s is not a permitted network type (%s)",
				ErrProhibitedNetwork,
				network,
				strings.Join(g.networks, ", "),
			)
		}
	}
...
func (g *Guardian) validatePort(port uint16) error {
	if g.ports == nil || slices.Contains(g.ports, port) {
		return nil
	}

	return fmt.Errorf("%w: %d is not a permitted port", ErrProhibitedPort, port)
}

func (g *Guardian) validateIP(ip netip.Addr) error {
	if ip.Is6() {
		return g.validateIPv6(ip)
	}

	return g.validateIPv4(ip)
}

func (g *Guardian) validateIPv6(ip netip.Addr) error {
	...
	if !IPv6GlobalUnicast.Contains(ip) {
		return fmt.Errorf(
			"%w: %s is not a permitted destination outside of the IPv6 Global Unicast range",
			ErrProhibitedIP,
			ip,
		)
	}
	...
	for _, net := range g.deniedv6Prefixes {
		if net.Contains(ip) {
			return fmt.Errorf(
				"%w: %s is not a permitted destination (denied by: %s)",
				ErrProhibitedIP,
				ip,
				net.String(),
			)
...
func (g *Guardian) validateIPv4(ip netip.Addr) error {
